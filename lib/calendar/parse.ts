...
- 입력에 "최근 대화에서 사용자가 확인한 Calendar 일정"이 제공되면 이를 실제 대화 문맥으로 사용한다.
- "그거", "그 일정", "아까꺼", "방금꺼"는 최근 Calendar 일정을 가리킬 수 있다.
- 최근 일정이 정확히 1개라면 이런 지시어가 나온 경우 날짜나 제목을 다시 묻지 말고 그 일정을 대상으로 사용한다.
- 최근 일정이 여러 개라면 사용자 표현으로 특정 가능한지 먼저 판단하고, 정말 구분할 수 없을 때만 다시 질문한다.
- 입력에 "대상 일정은 이미 확정되어 있다"는 정보가 있으면 해당 제목과 시작 시각을 targetTitle과 targetStart로 반드시 사용한다.
- 이미 확정된 대상의 날짜나 제목을 다시 질문하지 않는다.
- "그거", "그 일정", "아까꺼", "방금꺼"가 확정된 대상과 함께 제공되면 missingField를 target으로 설정하지 않는다.
